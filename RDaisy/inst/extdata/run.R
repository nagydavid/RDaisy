...
T_RunFile<-"F:/Jeroen/ZALF_Muncheberg.dai" # RunFile of daisy
T_path<-"C:/Program Files/Daisy 5.59/bin/daisy.exe" # path of Daisy executable
T_param<-"F:/Jeroen/ParamsToCalibrate.txt" #parameter matrix with min max and file names
T_Morris <- "F:/Jeroen/R_Daisy_package/Final/Morris_f.R"

#observation
T_obs<-fread("F:/Jeroen/obs/Soil_H2O_NO3_NH4.csv") 
T_obs[,date:=dmy(DATE_)] #convert Date_ to date format

T_obs_yield <- fread("F:/Jeroen/obs/Crop_ontogenesis.csv")
...
runDaisy(RunFile=T_RunFile, showLogFile=TRUE,PathToDaisy=T_path,ctrldaisy =  Daisy.control(sensitivity =FALSE,calib=FALSE, dflt = TRUE,costfunction=NULL,wdDir="F:/Jeroen",OutDir="output",interval=c(1993:1995),year=FALSE,All=FALSE,param_sens = NULL,p.config=T_param ,obs=T_obs_all))
...
f.cost(RunFile=T_RunFile, showLogFile=TRUE,PathToDaisy=T_path,ctrldaisy = Daisy.control(sensitivity = T,calib = FALSE, dflt = FALSE,costfunction = NULL,
       obs=T_obs_all, wdDir="F:/Jeroen/", OutDir="output",interval=1993:1995,year=TRUE,All=TRUE,ind=Sys.getpid(),param_sens = NULL,p=fread(T_param)$default, p.config=T_param))
...
Morris.SrC<-runDaisy(RunFile=T_RunFile, showLogFile=TRUE,PathToDaisy=T_path,ctrldaisy = Daisy.control(sensitivity=TRUE,calib = FALSE, dflt = FALSE,wdDir="F:/Jeroen",OutDir="output",interval=c(1993:1995),year=TRUE,All=TRUE, costfunction = DaisyMorris,param_sens = NULL,p.config=T_param ,obs=T_obs_all))
...
T_treshold<-fread("F:/Jeroen/S_treshold.csv")
