...
read.optim.0_30.swct <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (DEoptim == T){#If DEoptim is TRUE, then it reads output file which named after the running core(sys.getpid),
    ...
    daisyOut <- file.path(wdDir,OutDir,paste(Sys.getpid(), "_swct.txt", sep=""))
  } else {     #if false is named after the current rownumber of the sensitivity matrix
    daisyOut <- file.path(wdDir,OutDir,paste(ind, "_swct.txt", sep=""))
  }
  ...
  if (is.null(simOut) ) {
    ...
    x <- data.table(t(hydroGOF::gof(1:10,1:10)))
    x[x>=0]<-(1000)
    x[,year:=1000]
    x[,NSE:=1000]
    return(x)
    ...
    simOut[, date := lubridate::dmy(paste(paste(mday, month, year,sep="-")))]
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim0_30")],
                         obs[,.SD, .SDcols=c("date" ,"WATER_0_30_CM_VOL_PROZ")], all.y = T)
    ...
    SimObsSWC1 <- SimObsSWC[date>lubridate::dmy(paste("31-03",interval[1])) & date<=lubridate::dmy(paste("31-03",interval[2]))]
    SimObsSWC2 <- SimObsSWC[date>lubridate::dmy(paste("31-03",interval[2])) & date<=lubridate::dmy(paste("31-03",interval[3]))]
    #Calculating HydroGOF matrix
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim0_30, obs = SimObsSWC1$WATER_0_30_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim0_30, obs = SimObsSWC2$WATER_0_30_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
...
read.optim.30_60.swct <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (is.null(simOut) ) {
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim30_60")],
                       obs[,.SD, .SDcols=c("date" ,"WATER_30_60_CM_VOL_PROZ")], all.y = T)
    ...
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim30_60, obs = SimObsSWC1$WATER_30_60_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim30_60, obs = SimObsSWC2$WATER_30_60_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
...
read.optim.60_90.swct <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (is.null(simOut) ) {
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim60_90")],
                       obs[,.SD, .SDcols=c("date" ,"WATER_60_90_CM_VOL_PROZ")], all.y = T)

    SimObsSWC[, WATER_30_60_CM_VOL_PROZ:=WATER_60_90_CM_VOL_PROZ/100]
    ...
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim60_90, obs = SimObsSWC1$WATER_60_90_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim60_90, obs = SimObsSWC2$WATER_60_90_CM_VOL_PROZ,  norm = "maxmin",digits = 4)
...
read.optim.0_30.N <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (DEoptim == T){
    ...
    daisyOut <- file.path(wdDir,OutDir,paste(Sys.getpid(), "_soilNO3N.dlf", sep=""))
  } else {
    daisyOut <- file.path(wdDir,OutDir,paste(ind, "_soilNO3N.dlf", sep=""))
  }
  ...
  if (is.null(simOut) ) {
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim0_30")],
                       obs[,.SD, .SDcols=c("date" ,"NITRAT_0_30_CM_KG_HA")], all.y = T)

    SimObsSWC[, NITRATE_N_0_30_CM_g_cm2:=NITRAT_0_30_CM_KG_HA*0.226*10^-5]#http://newsroom.unl.edu/announce/beef/5891/33379
    SimObsSWC[, NITRATE_N_0_30_CM_g_cm3:=NITRATE_N_0_30_CM_g_cm2*30]
    ...
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim0_30, obs = SimObsSWC1$NITRATE_N_0_30_CM_g_cm3,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim0_30, obs = SimObsSWC2$NITRATE_N_0_30_CM_g_cm3,  norm = "maxmin",digits = 4)
...
read.optim.30_60.N <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (is.null(simOut) ) {
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim30_60")],
                       obs[,.SD, .SDcols=c("date" ,"NITRAT_30_60_CM_KG_HA")], all.y = T)

    SimObsSWC[, NITRATE_N_30_60_CM_g_cm2:=NITRAT_30_60_CM_KG_HA*0.226*10^-5]#http://newsroom.unl.edu/announce/beef/5891/33379
    SimObsSWC[, NITRATE_N_30_60_CM_g_cm3:=NITRATE_N_30_60_CM_g_cm2*30]
    ...
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim30_60, obs = SimObsSWC1$NITRATE_N_30_60_CM_g_cm3,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim30_60, obs = SimObsSWC2$NITRATE_N_30_60_CM_g_cm3,  norm = "maxmin",digits = 4)
...
read.optim.60_90.N <- function(ind, obs ,wdDir, OutDir, interval,year,DEoptim){
  ...
  if (is.null(simOut) ) {
    ...
    SimObsSWC <- merge(simOut[,.SD, .SDcols=c( "date", "sim60_90")],
                       obs[,.SD, .SDcols=c("date" ,"NITRAT_60_90_CM_KG_HA")], all.y = T)

    SimObsSWC[, NITRATE_N_60_90_CM_g_cm2:=NITRAT_60_90_CM_KG_HA*0.226*10^-5]#http://newsroom.unl.edu/announce/beef/5891/33379
    SimObsSWC[, NITRATE_N_60_90_CM_g_cm3:=NITRATE_N_60_90_CM_g_cm2*30]
    ...
    HydGOF1 <- hydroGOF::gof(sim = SimObsSWC1$sim60_90, obs = SimObsSWC1$NITRATE_N_60_90_CM_g_cm3,  norm = "maxmin",digits = 4)
    HydGOF2 <- hydroGOF::gof(sim = SimObsSWC2$sim60_90, obs = SimObsSWC2$NITRATE_N_60_90_CM_g_cm3,  norm = "maxmin",digits = 4)
