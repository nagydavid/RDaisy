...
f.cost <- function(p, p.config, RunFile,showLogFile,PathToDaisy,Morris,DEoptim,dflt,
                   costfunction,obs,wdDir,OutDir,interval,year,All,ind,param_sens) {
  ...
  updateParameters(p, p.config,RunFile,wdDir, Morris, DEoptim,dflt, ind,param_sens)
  #2. Run Daisy with the new input files
  runDaisy(p.config,RunFile,showLogFile=T ,PathToDaisy, Morris=F,DEoptim=F,dflt=F,costfunction=NULL,obs,wdDir,OutDir,interval=NULL,year=F,All=F,param_sens=NULL)
}